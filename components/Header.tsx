...
const Header = () => {
  ...
  const SendMail = async (e: any) => {
    ...
    const bemerkungen = form.elements.namedItem('bemerkungen') as HTMLInputElement
    ...
    const kinder = form.elements.namedItem('kinder') as HTMLInputElement
    ...
    if (!küche.checked && !buffet.checked && !service.checked && !bar.checked && !verkehr.checked && !kinder.checked && !sonstiges.checked) {
    ...
    text += `Kindereinsatz: ${kinder.checked ? 'Ja' : 'Nein'} \n`
    ...
    if (res.status === 200) {
      ...
      toast.success('Vielen Dank für deine Anmeldung! Wir werden uns zeitnah bei dir melden.')
    } else {
      ...
      <Container className="flex flex-wrap items-center justify-center sm:justify-between lg:flex-nowrap">
        ...
        <div className="modal modal-bottom sm:modal-middle">
          <div className="modal-box">
            ...
            <h3 className="font-bold text-red-600 dark:text-white text-lg">HelferIn werden</h3>
            <form className="mt-4" id="helfer-form">
              <div className="form-control w-full">
                <label className="label">
                  <span className="label-text">Wie heisst du?</span>
                </label>
                <input id="name" type="text" placeholder="Max Muster" className="input input-bordered w-full" required />
              </div>
              <div className="form-control w-full">
                <label className="label">
                  <span className="label-text">Deine E-Mail</span>
                </label>
                <input id="email" type="email" placeholder="max.muster@gmail.com" className="input input-bordered w-full" required />
              </div>
              <div className="form-control w-full">
                <label className="label">
                  <span className="label-text">Deine Telefonnummer</span>
                </label>
                <input id="phone" type="text" placeholder="0798000000" className="input input-bordered w-full" required />
              </div>
              <div className="form-control">
                <label className="label">
                  <span className="label-text">Bemerkungen</span>
                </label>
                <textarea id="bemerkungen" className="textarea textarea-bordered h-24" placeholder="Bemerkungen"></textarea>
              </div>
              ...
              <div className="form-control">
                <label className="label cursor-pointer">
                  <span className="label-text">Kindereinsätze am <b>Samstag</b> (Getränkewägeli, Marschmusik, Abzeichenverkauf, ...)</span>
                  <input id="kinder" type="checkbox" className="checkbox" />
                </label>
              </div>
              <div className="form-control">
                <label className="label cursor-pointer">
